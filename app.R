  ...
  shinyServer(function(input, output, session) {
    output$page3 <- renderUI({
      tagList(
        ...
        p("Using the Isoform Expression function, you could query a gene of your interest for its isoform profile across lung cell types. Enter the number for the top N most abundant isoforms of the queried gene to check the expression across 37 lung cell types (e.g., top 4 isoforms usually account for most of the total gene expression levels)."),
        p("You can select a specific cell type to check the isoform composition within the cell type. For efficient visualization, we set a cap for the number of isoforms at 37; > 90% of genes have less than 37 isoforms."),
        h3("isoQTL"),
        p('Using the isoQTL function, you could query significant isoQTLs for the SNP and transcript isoform of your interest in a specific lung cell type. Allelic box plots, a summary of all significant SNPs for the queried transcript isoform, and a summary of all the significant isoQTLs for the queried SNP for any isoform of the gene in any lung cell type are provided. Specific transcript IDs can be obtained in the search box above. IDs starting with "TALON" are novel isoforms identified by long-read sequencing in our dataset. For the variants, rs ID should be provided. For variants without an assigned rs ID, please use chr:pos in hg38 (e.g., chr1:145830810).'),
        h2("Short-read eQTL"),
        p("The functions of ", strong(em("Gene expression")), " and ", 
          strong(em("eQTL")), 
          "are under the Short-read eQTL tab and provide the results of cell-barcode matched single-cell short-read data."),
        h3("Gene Expression"),
        p("Using the Gene Expression function, you could query the normalized expression of genes of your interest across our 41 lung cell types. For plotting, cell types with more than 3,000 cells were downsampled to 3,000 cells, while cell types with less than or equal to 3,000 cells kept their original cell numbers. This was to maximize computation efficiency while still representing our dataset."),
        h3("eQTL"),
